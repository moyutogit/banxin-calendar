...
V0.1.5 已实现排班日历、官方节假日、原生闹钟、打卡与工时、工资统计、受控 AI
助理、本地备份恢复及分项隐私删除。
...
独立 Android 安装包在 GitHub Releases 的 `v0.1.5` 条目提供。该公开预览包使用
...
- AI 支持会话列表和开启新对话；相对日期先由 Asia/Shanghai 本地时间工具解析。
- AI 助理可在首轮对话配置名称、性格和回复风格，并只在用户明确要求时保存本地记忆。
- AI 工具校验失败或模型只返回思考过程时会持久显示明确答复，不会清空后只剩用户消息。
- 全应用操作提示使用顶部队列弹出；关闭按钮会立即关闭当前提示并清空待显示提示。
- 确定性排班优先级：手工覆盖 > 公司安排 > 官方节假日 > 排班规则 > 默认规则。
- Android 使用 AlarmClock 级精确调度、前台持续响铃、锁屏响铃页、触发回执与重启恢复；
  iOS 使用本地通知；不读取或修改系统时钟已有闹钟。
